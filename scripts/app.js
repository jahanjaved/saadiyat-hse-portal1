...
function ratingBadge(r) {
  const m = { Good: 'good', 'Needs Improvement': 'warn', Poor: 'bad', Critical: 'critical' };
  return badge(r || '—', m[r] || 'info');
}
...
function nav(active) {
  ...
  return `<div class="topbar"><div class="container"><div class="nav"><div class="brand">Saadiyat HSE Portal<small>Excel-to-website system</small></div>${items.map(([h, l]) => `<a href="${h}" class="${active === h ? 'active' : ''}">${l}</a>`).join('')}</div></div></div>`;
}

function shell(active, title, sub) {
  document.body.insertAdjacentHTML('afterbegin', nav(active));
  const root = document.getElementById('app');
  root.innerHTML = `<div class="container"><div class="hero"><div class="page-title"><div><h1>${title}</h1><p>${sub}</p></div></div></div><div id="content"></div><div class="footer">Prepared from the uploaded workbook. Weighted score fields in the source workbook contain broken references, so the website uses Raw Score % and all individual KPI fields.</div></div>`;
...
function buildRedFlagDatasets(data) {
  const monthly = (data.monthlyDashboard?.clusters || []).map(c => ({
    cluster: c.cluster,
    value: Number(c.redFlagsMonth || 0)
  }));

  const weekly = (data.weeklyDashboard?.clusters || []).map(c => ({
    cluster: c.cluster,
    value: Number(c.redFlagsWeek || 0)
  }));
...
function buildDashboard(data) {
  const c = shell(
    'index.html',
    'Full Aldar-style HSE Dashboard',
    `Selected week ${data.meta.selectedWeek} · Selected month ${data.meta.selectedMonth} · ${data.meta.inspectionCount} inspection records loaded`
  );
  ...
  c.innerHTML += `
    <div class="note">${data.meta.notes.map(n => `<div>• ${esc(n)}</div>`).join('')}</div>

    <div class="section grid cards">
      <div class="card"><h3>Total Inspections</h3><div class="metric">${data.meta.inspectionCount}</div></div>
      <div class="card"><h3>Selected Month Avg Raw</h3><div class="metric">${pct(monthAvg)}</div></div>
      <div class="card"><h3>All Records Avg Raw</h3><div class="metric">${pct(avgRaw)}</div></div>
      <div class="card"><h3>Critical Red Flags</h3><div class="metric">${redFlags}</div></div>
      <div class="card"><h3>Open CAPA Actions</h3><div class="metric">${data.capa.length}</div></div>
      <div class="card"><h3>Overdue CAPA</h3><div class="metric">${data.capa.filter(x => x.Overdue_Flag === 'YES').length}</div></div>
    </div>
      ...
      <div class="grid kpi-grid">
        ${data.gapAnalysis.kpis.map(k => `
          <div class="card kpi-card">
            ...
            <div class="small">Month avg / 5</div>
            <div class="bar"><span style="width:${((k.monthAvg || 0) / 5) * 100}%"></span></div>
            <div class="small" style="margin-top:8px">Week avg ${num(k.weekAvg)} · Gap to 5: ${num(k.monthGapTo5)}</div>
          </div>
        `).join('')}
      </div>
    </div>
    ...
    <div class="section grid two-col">
      <div class="card chart-card">
        <h2>Weekly Red Flags by Cluster</h2>
        <canvas id="weekRedFlagChart"></canvas>
      </div>
      <div class="card chart-card">
        <h2>Monthly Red Flags by Cluster</h2>
        <canvas id="monthRedFlagChart"></canvas>
      </div>
    </div>

    <div class="section">
      <h2>Red Flag Summary by Cluster</h2>
      <div id="redFlagTable"></div>
    </div>

    <div class="section">
      <h2>Red Flag Heatmap by Cluster</h2>
    ...
    <div class="section">
      <h2>Latest Inspection Records</h2>
  ...
  new Chart(document.getElementById('clusterChart'), {
    type: 'bar',
    data: {
      labels: data.weeklyDashboard.clusters.map(c => c.cluster),
      datasets: [
        {
          label: 'Week Avg Raw %',
          data: data.weeklyDashboard.clusters.map(c => c.weekAvgRawPct ? c.weekAvgRawPct * 100 : null)
        },
        {
          label: 'Month Avg Raw %',
          data: data.weeklyDashboard.clusters.map(c => c.monthAvgRawPct ? c.monthAvgRawPct * 100 : null)
        }
      ]
    },
    options: {
  ...
  new Chart(document.getElementById('kpiChart'), {
    ...
    data: {
      ...
      datasets: [
        { label: 'Selected Week', data: data.gapAnalysis.kpis.map(k => k.weekAvg) },
        { label: 'Selected Month', data: data.gapAnalysis.kpis.map(k => k.monthAvg) }
      ]
    },
    options: {
      responsive: true,
  ...
  new Chart(document.getElementById('weekRedFlagChart'), {
    ...
    data: {
      ...
      datasets: [{
        label: 'Weekly Red Flags',
        data: redFlagData.weekly.map(r => r.value)
      }]
    },
    options: {
      indexAxis: 'y',
      ...
      scales: {
        x: {
          beginAtZero: true,
          ticks: { color: '#bfd2e9', precision: 0 }
        },
        y: {
          ticks: { color: '#bfd2e9' }
        }
  ...
  new Chart(document.getElementById('monthRedFlagChart'), {
    ...
    data: {
      ...
      datasets: [{
        label: 'Monthly Red Flags',
        data: redFlagData.monthly.map(r => r.value)
      }]
    },
    options: {
      indexAxis: 'y',
...
function buildInspections(data) {
  const c = shell('inspections.html', 'Weekly Inspection Records', 'Full inspection records with all KPI fields from the workbook');
  c.innerHTML += `<div class="filters"><input id="q" placeholder="Search cluster, area, activity..." /><select id="clusterFilter"><option value="">All clusters</option>${data.meta.clusters.map(cl => `<option>${esc(cl)}</option>`).join('')}</select><select id="redFlagFilter"><option value="">All red flag status</option><option>YES</option><option>NO</option></select></div><div id="inspectionTable"></div>`;
  ...
  function draw() {
    ...
    const rows = data.weeklyInspections
      .filter(r => {
      ...
      .map(r => [
        ...
        num(r.Falling_Object_Prevention),
        esc(r.Excavation_Safety),
        num(r.Scaffolding_Compliance),
        num(r.PTW_Implementation),
        num(r.PTW_Field_Verification),
        esc(r.MSRA_Quality),
        num(r['Lifting/Precast Installation']),
        num(r.Traffic_Interface),
        num(r['Housekeeping/Waste Management']),
        num(r['Welfare Arrangement']),
        num(r.Fire_Readiness),
        num(r.Supervision_Subcontractor),
        num(r.Electrical_Tool_Safety),
        pct(r['Raw_Score_%']),
        redFlagBadge(r.Critical_Red_Flag),
        ratingBadge(r.Rating_Band),
        esc(r.Likely_Root_Cause),
...
function buildCapa(data) {
  const c = shell('capa.html', 'CAPA Tracking System', 'Corrective and preventive actions built from inspection findings');
  c.innerHTML += `<div id="capaTable"></div>`;
...
function buildGap(data) {
  const c = shell('gap-analysis.html', 'Gap Analysis', 'Weak KPI areas, gaps to target score 5, and likely root causes');
  c.innerHTML += `<div class="section grid two-col"><div class="card chart-card"><h2>Gap to 5 by KPI</h2><canvas id="gapChart"></canvas></div><div class="card chart-card"><h2>Root Cause Frequency</h2><canvas id="rootChart"></canvas></div></div><div class="section"><div id="gapTable"></div></div><div class="section"><div id="rootTable"></div></div>`;
  renderTable(document.getElementById('gapTable'), ['KPI', 'Week Avg', 'Month Avg', 'Week Gap to 5', 'Month Gap to 5', 'Month Focus'], data.gapAnalysis.kpis.map(k => [esc(k.label), num(k.weekAvg), num(k.monthAvg), num(k.weekGapTo5), num(k.monthGapTo5), (k.monthGapTo5 >= 2 ? badge('Immediate attention', 'critical') : badge('Monitor', 'warn'))]));
  ...
  new Chart(document.getElementById('gapChart'), {
    ...
    options: {
      responsive: true,
      plugins: { legend: { labels: { color: '#dce9f7' } } },
      scales: { x: { ticks: { color: '#bfd2e9' } }, y: { ticks: { color: '#bfd2e9' } } }
    }
  });
  new Chart(document.getElementById('rootChart'), {
    ...
    options: { responsive: true, plugins: { legend: { labels: { color: '#dce9f7' } } } }
  });
}

function buildSchedules(data) {
  const c = shell('schedules.html', 'April / May / June Schedules', 'All schedule sheets loaded from the workbook');
  ...
  function draw(month) {
    ...
    holder.innerHTML = `<div class="table-wrap"><table>${matrix.map((r, ri) => `<tr>${r.map(c => ri === 0 ? `<th>${esc(c ?? '')}</th>` : `<td>${esc(c ?? '')}</td>`).join('')}</tr>`).join('')}</table></div>`;
  }
...
function buildRaw(data) {
  const c = shell('raw-data.html', 'All Workbook Sheets', 'Every sheet from the Excel file is available here');
