...
function buildCombinedRedFlagDataset(data) {
  ...
  clusters.sort((a, b) => {
    const aw = Number((weekly.find(x => x.cluster === a) || {}).redFlagsWeek || 0);
    const am = Number((monthly.find(x => x.cluster === a) || {}).redFlagsMonth || 0);
    const bw = Number((weekly.find(x => x.cluster === b) || {}).redFlagsWeek || 0);
    const bm = Number((monthly.find(x => x.cluster === b) || {}).redFlagsMonth || 0);
    return (bm + bw) - (am + aw);
  });
...
function buildDashboard(data) {
  ...
  c.innerHTML += `
    ...
    <div class="section grid two-col">
      ...
      <div class="card chart-card"><h2>KPI Performance</h2><canvas id="kpiChart"></canvas></div>
    </div>
  ...
  new Chart(document.getElementById('kpiChart'), {
    ...
    data: {
      ...
      datasets: [
        { label: 'Week', data: kpis.map(k => k.weekAvg) },
        { label: 'Month', data: kpis.map(k => k.monthAvg) }
      ]
    },
    options: {
      ...baseChartOptions(),
