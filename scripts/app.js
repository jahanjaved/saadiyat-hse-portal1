...
function buildDashboard(data) {
  ...
  c.innerHTML += `
    ...
    <div class="section grid two-col">
      <div class="card">
        <h2>Open vs Overdue Actions</h2>
        <div class="small">Each bar shows total open actions, with overdue actions highlighted inside.</div>
  ...
  const kpiRows = (data.gapAnalysis?.kpis || [])
    .slice()
    .sort((a, b) => (Number(a.monthAvg || 0) - Number(b.monthAvg || 0)) || a.label.localeCompare(b.label));
  kpiHolder.innerHTML = kpiRows.map(k => {
    const week = Number(k.weekAvg || 0);
    const month = Number(k.monthAvg || 0);
