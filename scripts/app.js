    ...
    new Chart(document.getElementById(cardId), {
      ...
      data: {
        ...
        datasets: [
          {
            label: 'Week',
            data: cluster.weekKpis.map(v => Number.isFinite(Number(v)) ? Number(v) : null),
            backgroundColor: cluster.weekKpis.map(getBarColorByKpiScore),
            borderColor: cluster.weekKpis.map(getBarColorByKpiScore),
            borderWidth: 1,
            categoryPercentage: 0.72,
            barPercentage: 0.88
          },
          {
            label: 'Month',
            data: cluster.monthKpis.map(v => Number.isFinite(Number(v)) ? Number(v) : null),
            backgroundColor: cluster.monthKpis.map(getBarColorByKpiScore),
            borderColor: cluster.monthKpis.map(getBarColorByKpiScore),
            borderWidth: 1,
            categoryPercentage: 0.72,
            barPercentage: 0.88
          }
