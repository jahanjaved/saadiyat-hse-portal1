...
function scoreColor(v) {
  const n = safeNum(v);
  if (n === null) return 'rgba(125,169,255,0.65)';
  if (n < 2.5) return 'rgba(255,93,122,0.80)';
  if (n < 3.5) return 'rgba(241,184,75,0.80)';
  return 'rgba(31,190,122,0.80)';
}
...
const clusterBarValuePlugin = {
  id: 'clusterBarValuePlugin',
  afterDatasetsDraw(chart) {
    ...
    chart.data.datasets.forEach((dataset, datasetIndex) => {
      ...
      meta.data.forEach((bar, index) => {
        ...
        const boxHeight = 26;
        let boxY = topY + 16;
        if (barHeight < 42) boxY = topY - 14;
        const boxX = centerX - boxWidth / 2;
        const radius = 8;
        const color = typeof dataset.backgroundColor === 'function'
          ? dataset.backgroundColor({ raw: n, dataIndex: index, dataset, chart })
          : Array.isArray(dataset.backgroundColor)
            ? dataset.backgroundColor[index]
            : dataset.backgroundColor;
        ctx.fillStyle = color || scoreColor(n);
        ctx.beginPath();
        ctx.moveTo(boxX + radius, boxY - boxHeight / 2);
        ctx.lineTo(boxX + boxWidth - radius, boxY - boxHeight / 2);
        ctx.quadraticCurveTo(boxX + boxWidth, boxY - boxHeight / 2, boxX + boxWidth, boxY - boxHeight / 2 + radius);
        ctx.lineTo(boxX + boxWidth, boxY + boxHeight / 2 - radius);
        ctx.quadraticCurveTo(boxX + boxWidth, boxY + boxHeight / 2, boxX + boxWidth - radius, boxY + boxHeight / 2);
        ctx.lineTo(boxX + radius, boxY + boxHeight / 2);
        ctx.quadraticCurveTo(boxX, boxY + boxHeight / 2, boxX, boxY + boxHeight / 2 - radius);
        ctx.lineTo(boxX, boxY - boxHeight / 2 + radius);
        ctx.quadraticCurveTo(boxX, boxY - boxHeight / 2, boxX + radius, boxY - boxHeight / 2);
        ctx.closePath();
        ctx.fill();
        ctx.fillStyle = '#ffffff';
        ctx.fillText(text, centerX, boxY + 1);
      });
    });
    ctx.restore();
  }
...
function buildClusterPerformance(data) {
  ...
  summary.forEach((r, idx) => {
    ...
    card.innerHTML = `
      ...
      <div class="chart-card" style="margin-top:14px;height:430px;"><h3>KPI Profile</h3><canvas id="${canvasId}"></canvas></div>
    `;
    ...
    new Chart(document.getElementById(canvasId), {
      ...
      data: {
        ...
        datasets: [
          {
            label: 'Week',
            data: kpis.map(([, field]) => r.kpiWeek[field]),
            backgroundColor: (ctx) => scoreColor(ctx.raw),
            borderColor: (ctx) => scoreColor(ctx.raw),
            borderWidth: 1,
            barPercentage: 0.82,
          ...
          {
            label: 'Month',
            data: kpis.map(([, field]) => r.kpiMonth[field]),
            backgroundColor: 'rgba(125,169,255,0.32)',
            borderColor: 'rgba(125,169,255,0.70)',
            borderWidth: 1,
            barPercentage: 0.82,
            categoryPercentage: 0.72
          }
      ...
      options: {
        ...
        plugins: {
          ...
          tooltip: {
            callbacks: { label: (ctx) => `${ctx.dataset.label}: ${safeNum(ctx.raw) === null ? '—' : Number(ctx.raw).toFixed(1)}` }
          }
        },
        scales: {
