...
if (typeof Chart === 'undefined') { window.Chart = class { static register() {} constructor() {} }; }
...
const PREVENTIVE_ACTION_PATTERNS = [
  {
    match: /ptw must be controlled by the main contractor|permit compliance/i,
    detail: "Permit-to-work control shall remain fully under the authority of the main contractor, with no work allowed to proceed unless the permit is valid, site conditions are verified, and all required control measures are in place. Close supervision shall be maintained during execution to ensure subcontractors work strictly within the approved permit scope and project OSH requirements."
  },
  {
    match: /close supervision of sub-?contractors|provide competent supervision|strict supervision/i,
    detail: "Close and competent supervision shall be maintained by the responsible construction and HSE team to monitor the activities of all subcontractors, verify compliance with approved method statements and risk assessments, and immediately correct any unsafe act, unsafe condition, or deviation from the project requirements."
  },
  {
    match: /conduct coordination meetings|pre-?plan|look-?ahead planning|simops/i,
    detail: "The activities shall be pre-planned through regular coordination meetings and short-term look-ahead planning so that work interfaces, simultaneous operations, access requirements, and resource needs are identified in advance. This shall help avoid conflicts, reduce unsafe overlap between trades, and ensure that the work is executed in a controlled and organized manner."
  },
  {
    match: /waste collection|housekeeping|adequate facilities for waste collection/i,
    detail: "Adequate housekeeping arrangements and waste collection facilities shall be provided and maintained at all times. Waste shall be removed on a routine basis, temporary storage areas shall be clearly identified, and all work locations shall be kept clean and unobstructed to prevent slip, trip, fire, and access hazards."
  },
  {
    match: /training and awareness|regular training sessions/i,
    detail: "Regular training and awareness sessions shall be conducted for supervisors, operatives, and subcontractor teams to reinforce project OSH requirements, hazard identification, permit controls, and the safe execution of high-risk activities. The effectiveness of the training shall be monitored through routine site inspections and follow-up actions."
  },
  {
    match: /scaffold|scaffolding inspector|distribution board inspection/i,
    detail: "All scaffolding and associated temporary access arrangements shall be inspected by a competent scaffolding inspector at the required frequency, and any distribution boards or temporary electrical equipment within the work area shall also be routinely checked to ensure they remain safe, compliant, and suitable for continued use."
  },
  {
    match: /safe and proper access platforms|working platforms|mobile/i,
    detail: "Safe and proper access arrangements, including working platforms, access scaffolds, and mobile access equipment where applicable, shall be provided and maintained in accordance with the approved method statement and risk assessment. These arrangements shall be inspected regularly to ensure stability, suitability, and safe access for the workforce."
  },
  {
    match: /hot work permit|fire watch/i,
    detail: "Hot work activities shall be carried out only under a valid hot work permit, with continuous supervision, appropriate fire extinguishing equipment, and a dedicated fire watch where required. Combustible materials shall be controlled or removed from the area, and the workplace shall remain monitored until the risk of ignition has fully passed."
  }
];
...
function expandPreventiveActionLine(line) {
  ...
  if (clean.length < 80) {
    return `${clean.charAt(0).toUpperCase()}${clean.slice(1)} shall be implemented by the responsible construction and supervision team, monitored on site, and sustained through routine inspection and close follow-up to ensure full compliance with project OSH requirements.`;
  }
  return clean;
}

function humanizePreventiveAction(value) {
  const lines = splitActionLines(value);
...
function preprocessInspectionTextRows(rows) {
  if (!Array.isArray(rows)) return [];
  return rows.map(row => {
    const next = { ...row };
    if (next.Preventive_Action_Required) {
      next.Preventive_Action_Required_Detailed = humanizePreventiveAction(next.Preventive_Action_Required);
      next.Preventive_Action_Required = next.Preventive_Action_Required_Detailed;
    }
    return next;
  });
}

function preprocessCapaRows(rows) {
  if (!Array.isArray(rows)) return [];
  return rows.map(row => {
    const next = { ...row };
    if (next.Preventive_Action) {
      next.Preventive_Action_Detailed = humanizePreventiveAction(next.Preventive_Action);
      next.Preventive_Action = next.Preventive_Action_Detailed;
    }
    return next;
  });
}
...
function preprocessData(data) {
  ...
  const next = { ...data };
  next.meta = next.meta || { selectedWeek: '—', selectedMonth: '—', inspectionCount: 0, clusters: [] };
  next.weeklyInspections = Array.isArray(next.weeklyInspections) ? next.weeklyInspections : [];
  next.weeklyDashboard = next.weeklyDashboard || { clusters: [] };
  next.monthlyDashboard = next.monthlyDashboard || { clusters: [] };
  next.gapAnalysis = next.gapAnalysis || { kpis: [], rootCauses: [] };
  next.heatmap = Array.isArray(next.heatmap) ? next.heatmap : [];
  next.capa = Array.isArray(next.capa) ? next.capa : [];
  next.schedules = next.schedules || {};
  next.rawSheets = next.rawSheets || {};
  next.weeklyInspections = preprocessInspectionTextRows(data.weeklyInspections);
  next.capa = preprocessCapaRows(data.capa);
  return next;
}

// Exposed for report/export code so Word output can use the same human-style wording.
...
function nav(active) {
  ...
  return `
    <div class="topbar">
      <div class="container">
        <div class="nav">
          <div class="brand"><span>Saadiyat Lagoons HSE</span><small>Executive performance dashboard</small></div>
...
function buildInspections(data) {
  ...
  function draw() {
    ...
    const rows = (data.weeklyInspections || []).filter(r => {
      ...
      esc(r.Inspection_Date), esc(r.Week_No), esc(r.Package), esc(r.Cluster), esc(r.Area_or_Villa), esc(r.Main_High_Risk_Activity), esc(r.Stop_Work || '—'), num(r.Work_at_Height), num(r.Edge_Protection), num(r.Falling_Object_Prevention), esc(r.Excavation_Safety), num(r.Scaffolding_Compliance), num(r.PTW_Implementation), num(r.PTW_Field_Verification), num(r.MSRA_Quality), num(r['Lifting/Precast Installation'] ?? r['Lifting_Precast_Installation']), num(r.Traffic_Interface), num(r['Housekeeping/Waste Management'] ?? r['Housekeeping_Waste_Management']), num(r['Welfare Arrangement'] ?? r['Welfare_Arrangement']), num(r.Fire_Readiness), num(r.Supervision_Subcontractor), num(r.Electrical_Tool_Safety), pct(Number(r['Raw_Score_%']) || Number(r.Raw_Score_Pct)), redFlagBadge(r.Critical_Red_Flag), ratingBadge(r.Rating_Band), esc(r.Likely_Root_Cause), `<div class="pre">${nl(r.Top_3_Gaps_Observed)}</div>`, `<div class="pre">${nl(r.Immediate_Action_Taken)}</div>`, `<div class="pre">${nl(r.Preventive_Action_Required_Detailed || r.Preventive_Action_Required)}</div>`
    ]);
    holder.innerHTML = '';
...
function buildCapa(data) {
  ...
  renderTable(document.getElementById('capaTable'), ['Action ID', 'Date Raised', 'Cluster', 'Package', 'KPI Area', 'Root Cause', 'Finding', 'Immediate Action', 'Preventive Action', 'Owner', 'Target', 'Overdue'], (data.capa || []).map(r => [esc(r.Action_ID), esc(r.Date_Raised), esc(r.Cluster), esc(r.Package), esc(r.KPI_Area), esc(r.Root_Cause), `<div class="pre">${nl(r.Finding)}</div>`, `<div class="pre">${nl(r.Immediate_Action)}</div>`, `<div class="pre">${nl(r.Preventive_Action_Detailed || r.Preventive_Action)}</div>`, esc(r.Action_Owner), esc(r.Target_Date), String(r.Overdue_Flag).toUpperCase() === 'YES' ? badge('Overdue', 'critical') : badge('On time', 'good')]));
}
...
(async function () {
  try {
    ...
    if (page === 'cluster-performance.html') buildClusterPerformance(data);
