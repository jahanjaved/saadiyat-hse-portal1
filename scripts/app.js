...
function buildDashboard(data) {
  ...
  renderTable(document.getElementById('openOverdueTable'), ['Cluster', 'Open', 'Overdue'], openVsOverdue.map(r => [
    esc(r.cluster),
    badge(r.open, r.open > 0 ? 'warn' : 'good'),
    badge(r.overdue, r.overdue > 0 ? 'critical' : 'good')
  ]));

  renderTable(document.getElementById('redFlagTable'), ['Cluster', 'Week Red Flags', 'Month Red Flags', 'Week Inspections', 'Month Inspections'],
  ...
  renderTable(document.getElementById('latestTable'), ['Date', 'Cluster', 'Area / Villa', 'Main Activity', 'Raw Score', 'Red Flag', 'Rating'],
    inspections.slice().sort((a, b) => String(b.Inspection_Date).localeCompare(String(a.Inspection_Date))).map(r => [
  ...
  new Chart(document.getElementById('weekRedFlagChart'), {
    type: 'bar',
    data: { labels: redFlagData.weekly.map(r => r.cluster), datasets: [{ label: 'Weekly Red Flags', data: redFlagData.weekly.map(r => r.value) }] },
  ...
  new Chart(document.getElementById('monthRedFlagChart'), {
    type: 'bar',
    data: { labels: redFlagData.monthly.map(r => r.cluster), datasets: [{ label: 'Monthly Red Flags', data: redFlagData.monthly.map(r => r.value) }] },
    options: { ...baseChartOptions(), indexAxis: 'y', scales: { x: { beginAtZero: true, ticks: { color: '#bfd2e9', precision: 0 } }, y: { ticks: { color: '#bfd2e9' } } } }
  });
  ...
  new Chart(document.getElementById('openVsOverdueChart'), {
    ...
    options: { ...baseChartOptions(), scales: { x: { ticks: { color: '#bfd2e9' } }, y: { beginAtZero: true, ticks: { color: '#bfd2e9', precision: 0 } } } }
  });
}
...
function buildInspections(data) {
  ...
  function draw() {
    ...
    const rows = (data.weeklyInspections || []).filter(r => {
      ...
      esc(r.Inspection_Date), esc(r.Week_No), esc(r.Package), esc(r.Cluster), esc(r.Area_or_Villa), esc(r.Main_High_Risk_Activity), esc(r.Stop_Work || '—'), num(r.Work_at_Height), num(r.Edge_Protection), num(r.Falling_Object_Prevention), esc(r.Excavation_Safety), num(r.Scaffolding_Compliance), num(r.PTW_Implementation), num(r.PTW_Field_Verification), esc(r.MSRA_Quality), num(r['Lifting/Precast Installation']), num(r.Traffic_Interface), num(r['Housekeeping/Waste Management']), num(r['Welfare Arrangement']), num(r.Fire_Readiness), num(r.Supervision_Subcontractor), num(r.Electrical_Tool_Safety), pct(r['Raw_Score_%']), redFlagBadge(r.Critical_Red_Flag), ratingBadge(r.Rating_Band), esc(r.Likely_Root_Cause), `<div class="pre">${nl(r.Top_3_Gaps_Observed)}</div>`, `<div class="pre">${nl(r.Immediate_Action_Taken)}</div>`, `<div class="pre">${nl(r.Preventive_Action_Required)}</div>`
    ]);
