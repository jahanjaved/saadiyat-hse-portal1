...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  const topCluster = [...dRows]
    .filter(r => r['Week Avg Raw %'] != null)
    .sort((a,b)=>(b['Week Avg Raw %']||0)-(a['Week Avg Raw %']||0))[0];
  const attentionCluster = [...iRows]
    .filter(r => /yes/i.test(String(r['Critical_Red_Flag'] || '')))
    .sort((a,b)=>(a['Raw_Score_%']||9)-(b['Raw_Score_%']||9))[0];
